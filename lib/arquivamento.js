 ...
 * ⚠️ A ÚNICA CÓPIA DA REGRA. `ultimoEvento` é 'ci_decidiu' | 'ci_reabriu' | null.
 */
function bloqueioDeFatos(f, ultimoEvento) {
  ...
  if (f.com_analista && ultimoEvento === 'ci_reabriu') return 'O Controle Interno reabriu esta parcial: '
...
function bloqueio(pcs, ultimoCi) {
  return bloqueioDeFatos(fatosDe(pcs), ultimoCi ? ultimoCi.evento : null);
}
