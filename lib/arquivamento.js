...
// ⚠️ O SETOR DO C.I. TEM DONO, e e a `lib/sgpe-situacao`: a mesma pergunta em JS e em SQL,
// para a leitura do estado e para a rota que arquiva nao divergirem.
...
// ⚠️ O `LEFT JOIN` COM A SITUACAO DO SGPe ENTRA AQUI PORQUE A TRAVA E DA ESCRITA TAMBEM.
// Sem ele, a rota de arquivar nao teria como saber que o processo esta no C.I. — e a trava
// viveria so na tela, que e onde ela NAO pode viver (a tela nao decide).
//
// ⚠️ E O `FOR UPDATE` FICA SO NA TABELA DAS PCs (`FOR UPDATE OF p`): trancar a linha do cache
// do SGPe nao protege nada e brigaria com o job do rodizio, que escreve nela a cada hora.
const SQL_PARCELA = `
  SELECT p.codigo_pc, p.tr, p.parcial_num, p.setorial_id, p.tipo, p.analista_id, p.analista_nome,
         p.baixada, p.status, p.situacao_atual, p.enviado_ci, p.ci_situacao, p.origem_baixa, p.parecer_tipo,
         p.arquivada, p.arquivada_em::text AS arquivada_em, p.arquivada_por, p.obs_arquivamento,
         p.processo_pc,
         p.baixa_secretario_em::text AS baixa_secretario_em, p.baixa_secretario_por,
         p.baixa_secretario_registrada_em::text AS baixa_secretario_registrada_em,
         s.setor_sigla AS sgpe_setor, s.setor_nome AS sgpe_setor_nome,
         s.dias_no_setor AS sgpe_dias, s.desde::text AS sgpe_desde
    FROM prestacoes_contas p
    LEFT JOIN sgpe_situacao s
           ON ${CHAVE_PROC_SQL("(s.sigla || ' ' || s.numero_oficial::text || '/' || s.ano::text)")}
            = ${CHAVE_PROC_SQL("coalesce(p.processo_pc,'')")}
   WHERE p.setorial_id = $1 AND p.tr = $2 AND p.parcial_num = $3 AND ${inval.ativa('p')}
...
function fatosDe(pcs) {
  ...
  return {
    ...
    // ⚠️ BASTA UMA PC DA PARCELA COM O PROCESSO NO C.I. para travar a parcela inteira: elas
    // arquivam juntas, e arquivar metade nao existe. A primeira que responder serve — o
    // processo costuma ser o mesmo nas irmas.
    ci_no_sgpe: naoDevolvido(pcs),
  };
}

/**
 * A PC cujo processo esta, AGORA, dentro do Controle Interno no SGPe — ou null.
 *
 * ⚠️ QUEM DIZ O QUE E "SETOR DO C.I." E A `lib/sgpe-situacao`, nao esta funcao: e a mesma
 * resposta que o SQL do estado usa, e duas definicoes divergiriam no dia em que houvesse um
 * segundo setor de controle.
 */
function naoDevolvido(pcs) {
  const p = (pcs || []).find((x) => sgpeSit.ehSetorCI(x.sgpe_setor));
  return p ? {
    processo: p.processo_pc || '', setor: p.sgpe_setor,
    setor_nome: p.sgpe_setor_nome || null,
    dias: p.sgpe_dias == null ? null : Number(p.sgpe_dias),
    desde: p.sgpe_desde ? String(p.sgpe_desde).slice(0, 10).split('-').reverse().join('/') : null,
  } : null;
}
...
function bloqueioDeFatos(f) {
  ...
  // ⚠️ O PROCESSO AINDA ESTA COM O C.I. — a trava que faltava (23/09/2026).
  //
  // O sistema dizia "pronta para arquivar" e afirmava o acordo do C.I. sobre parciais cujo
  // processo estava, naquele instante, DENTRO do Controle Interno no SGPe. Medido em 23/09:
  // **634 das 797** parciais prontas, em 186 TRs, uma delas parada no C.I. ha 167 dias. A
  // situacao `encerrado` que sustentava a frase veio do UPDATE em massa de 16/08, nao do C.I.
  //
  // ⚠️ E QUEM ARQUIVASSE ALI ESTARIA ENCERRANDO O QUE O C.I. AINDA NAO DEVOLVEU. Por isso a
  // regra entra AQUI e nao na tela: e esta funcao que a leitura do estado e a rota
  // `POST /parcela/arquivar` consultam — uma regra, dois caminhos.
  //
  // ⚠️ SEM LEITURA DO SGPe, NADA MUDA. `ci_no_sgpe` so vem preenchido quando ha situacao lida
  // para o processo; sem ela nao da para afirmar onde ele esta, e afirmar seria o mesmo erro
  // na direcao contraria. Ordem do Richard: nao inventar.
  if (f.ci_no_sgpe) {
    const d = f.ci_no_sgpe;
    return `O processo ${d.processo} está no Controle Interno no SGPe (${d.setor})`
      + `${d.desde ? ` desde ${d.desde}` : ''}${d.dias != null ? ` — ${d.dias} dia${d.dias === 1 ? '' : 's'}` : ''}. `
      + 'Enquanto o C.I. não devolver o processo com o parecer, esta parcial não se arquiva.';
  }
...
function cteEstado(filtroTr = 'true') {
  ...
  -- O SETOR DO SGPe ENTRA POR CTE PROPRIA, e nao por JOIN dentro do arq_parc. Aquela CTE faz
  -- COUNT(*) AS n_pcs e monta as listas de codigos: uma linha de situacao repetida pela chave
  -- normalizada multiplicaria as PCs e inflaria numero que a tela mostra. Aqui o DISTINCT ON
  -- garante uma linha por parcela, e o que sobra do JOIN nao atravessa.
  -- E O OBJETO SAI PRONTO DO BANCO, no mesmo formato do naoDevolvido: e o que faz a leitura e
  -- o arquivar cairem na mesma bloqueioDeFatos, sem uma segunda copia da regra.
  arq_sgpe AS (
    SELECT DISTINCT ON (q.setorial_id, q.tr, q.parcial_num)
           q.setorial_id AS s, q.tr AS t, q.parcial_num AS n,
           jsonb_build_object(
             'processo', coalesce(q.processo_pc, ''),
             'setor', sg.setor_sigla, 'setor_nome', sg.setor_nome,
             'dias', sg.dias_no_setor, 'desde', to_char(sg.desde, 'DD/MM/YYYY'),
             -- checado_em e timestamptz: UM AT TIME ZONE ja leva para o horario de Brasilia. O par
             -- 'UTC' + 'America/Sao_Paulo' do arq_parc existe porque data_baixa NAO tem fuso.
             'lido_em', to_char(sg.checado_em AT TIME ZONE 'America/Sao_Paulo',
                                'DD/MM/YYYY HH24:MI')) AS ci_no_sgpe
      FROM prestacoes_contas q
      JOIN sgpe_situacao sg
        ON ${CHAVE_PROC_SQL("(sg.sigla || ' ' || sg.numero_oficial::text || '/' || sg.ano::text)")}
         = ${CHAVE_PROC_SQL("coalesce(q.processo_pc,'')")}
     WHERE ${inval.ativa('q')} AND (${filtroTr}) AND ${sgpeSit.sqlSetorCI('sg.setor_sigla')}
     ORDER BY q.setorial_id, q.tr, q.parcial_num, q.codigo_pc),
  arq_tr AS (
    SELECT s, t,
...
function sqlEstado(filtroTr) {
  return `WITH ${cteEstado(filtroTr)}
    SELECT ap.*, ac.evento AS ci_ultimo_evento, ac.em AS ci_ultimo_em,
      ...
      LEFT JOIN arq_sgpe asg ON asg.t = ap.t AND asg.n IS NOT DISTINCT FROM ap.n AND asg.s IS NOT DISTINCT FROM ap.s`;
}
...
function estadoDaLinha(r) {
  ...
  const base = {
    ...
    // ⚠️ ONDE O PROCESSO ESTA, AGORA, NO SGPe — e null quando ele nao esta no C.I. OU quando
    // ninguem leu o SGPe ainda. A tela precisa do objeto inteiro (setor, desde, dias, e a hora
    // da leitura) para dizer de onde tirou a afirmacao: o motivo sozinho nao da o link.
    ci_no_sgpe: r.ci_no_sgpe || null,
  };
...
async function anexarEstado(db, rows) {
  ...
  for (const r of est) {
    ...
    mapa.set(`${r.s}|${r.t}|${r.n}`, {
      ...
      ci_complemento: e.ci_complemento, ci_no_sgpe: e.ci_no_sgpe,
    });
  }
