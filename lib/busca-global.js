...
function validar(b) {
  ...
  if (!t) return 'Digite o que procura.';
  if (t.length < TERMO_MIN) return `Digite ao menos ${TERMO_MIN} caracteres.`;
...
function montarCards(rows, casaram, hojeIso, devolvidaEm) {
  ...
  return [...porTr.values()].map(c => {
    ...
    return {
      ...
      parciais_casaram: parciais.filter(p => p.casou).length,
      parciais,
    };
  })
...
module.exports = { CORTE_PRAZO, TERMO_MIN, MAX_TRS, condicaoBusca,
