 ...
 */
function condicaoTr(tr, values, proximo) {
  const t = String(tr == null ? '' : tr).trim();
  if (!t) return null;
  const p = `$${proximo}`;
  values.push(`%${semAcento(t).replace(/[%_]/g, (m) => '\\' + m)}%`);
  return { condicao: `tr ILIKE ${p}`, proximo: proximo + 1 };
}
 ...
 */
function condicaoProcesso(processo, values, proximo) {
  ...
  const partes = CAMPOS_PROCESSO.map(
    (c) => `position(${p} in ${CHAVE_PROC_SQL(`coalesce(${c},'')`)}) > 0`);
  return { condicao: `(${partes.join(' OR ')})`, proximo: i };
}
