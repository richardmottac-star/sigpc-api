...
//
// ⚠️ `ci_tecnico_id` E `ci_encerrado_por` CONTINUAM SENDO PERGUNTAS DIFERENTES, e agora a
// diferença aparece na devolução: uma PC devolvida para correção leva o técnico que a
// devolveu em `ci_tecnico_id` e **não tem** `ci_encerrado_por`, porque não encerrou nada.
// Numa encerrada os dois existem, e apontam para a mesma pessoa.
//
// ⚠️ E ESTA LIB NUNCA TOCA NA BAIXA. Nem a devolução à fila, nem o passar a outro mexem em
// `baixada`, `data_baixa`, `enviado_ci` ou `ci_situacao`: quem está com a PC é uma pergunta,
// e onde ela está no ciclo é outra. Há teste que falha se um UPDATE daqui mencionar qualquer
// uma das quatro.
//
 ...
 *
 * O `tecnicoId` continua entrando para o caso da PC que **voltou** de uma rodada de correção
 * e já traz dono: qualquer um dos três continua podendo decidir, porque são três pessoas na
 * mesma fila e travar por dono só criaria demanda parada quando alguém faltasse.
 *
...
//
// Ela marcava a PC como do técnico no instante em que ele a EXPANDIA na tela. A ideia era
// coordenar os três — quem abriu, ficou —, e o efeito foi outro: **olhar virou tomar**. Não
// havia como examinar um valor, um processo SGPe ou o parecer da analista sem se declarar
// responsável, e quem abrisse por engano ficava com o nome ali. Aconteceu no primeiro dia,
// com o superadmin, numa PC que ele não analisa.
//
// ⚠️ A POSSE PASSOU A SER CONSEQUÊNCIA DO PARECER, e não condição dele: `ci.decidir` grava
// `ci_tecnico_id` no mesmo UPDATE da decisão. Sem parecer, a PC continua na fila e sem nome
// de técnico — e fechar sem decidir não deixa rastro nenhum.
//
// ⚠️ E NÃO FOI SUBSTITUÍDA POR UMA ROTA DE "ASSUMIR". Um botão de assumir traria de volta o
// mesmo par de estados — pego / não pego — com um clique a mais. O que a fila precisa saber
// é quem JÁ decidiu, e isso o parecer responde sozinho.
//
// A rota `POST /ci/pc/abrir` saiu junto. `devolver` e `passar` continuam, para a PC que
// voltou de uma rodada de correção e já traz dono.
