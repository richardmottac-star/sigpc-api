 ...
 * de menu dele funcionar. O menu abre a Fila para `coordenador` e `superadmin`; com o
 * coordenador fora desta lista, ele veria o item, clicaria e leria
 * "Esta fila é do Controle Interno" — a tela que aceita ser aberta e não responde, que é a
 * armadilha 15 vestida de autorização. Decisão do Richard: coordenador e superadmin veem a
 * Fila COMO os técnicos do C.I.
 *
 * ⚠️ E "como os técnicos" inclui as três ações: esta função guarda a leitura E as escritas.
 * Liberar só a leitura devolveria o mesmo defeito um passo adiante — a lista abriria e os
 * botões dariam 403.
 */
function podeAgir(u) {
  if (!u) return false;
...
/**
 * ⚠️ ABRIR É SÓ LEITURA — QUEM DECIDE É QUEM ASSUMIU.  (25/08/2026, decisão do Richard)
 *
 * O técnico precisava abrir a TR para examinar antes de decidir se assume: ver as parcelas,
 * o parecer da analista, o processo SGPe. Só que "abrir" e "decidir" viraram a mesma porta —
 * e uma TR que qualquer um decide sem assumir é a fila sem dono de novo, com um passo a mais.
 *
 * A regra: você decide a TR que é SUA.
 *   · TR livre        → ninguém decide. Assuma primeiro.
 *   · TR de outro     → abre e vê; quem decide é quem está com ela.
 ...
 */
function podeDecidir(quem, responsavelId) {
  if (!quem) return false;
  if (papel.perfilEfetivo(quem) === 'superadmin') return true;
  return !!responsavelId && Number(responsavelId) === Number(quem.id);
}

/** O motivo da recusa, para a tela poder dizer POR QUE em vez de só apagar o botão. */
function motivoNaoDecide(responsavelNome) {
  return responsavelNome
    ? `Esta TR está com ${responsavelNome}. Peça que ela passe a demanda para você.`
    : 'Assuma a demanda para poder decidir.';
}
...
module.exports = {
  ...
  diasEspera, faixaEspera, podeAgir, podeDecidir, motivoNaoDecide, validarMotivo,
  resumir, assumir, devolver, passar,
};
