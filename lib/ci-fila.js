...
const SQL_RESUMO = `
  SELECT
   ...
   WHERE p.ci_situacao IS NOT NULL AND ${inval.ativa('p')}`;
 ...
 * PC que existe.
 */
function montarFiltro({ chip, meuId, q, sgpe, tr, processo, analista_id, espera }) {
  ...
  const params = [];
  // ⚠️ A PARCELA INVALIDADA SAI DA FILA E DA CONTAGEM. Aqui, e nao em sqlLista/sqlContar
  // separados: os dois leem este mesmo filtro, e duas copias divergiriam na primeira
  // mudanca — o total diria uma coisa e a lista mostraria outra.
  const w = [`p.ci_situacao IS NOT NULL`, inval.ativa('p')];
...

  {
    ...
    // ⚠️ `tr` E `processo` SOMAM COM O `q`, e NÃO com o `sgpe` (31/08/2026). Eles entram
    // depois do `return` antecipado lá em cima, de propósito: o bloco do `sgpe` é o "leve-me a
    // este processo", que ignora o chip por decisão do Richard e continua ignorando. Estes
    // dois são as caixas da BARRA DE FILTRO, que estreitam o recorte aberto.
    //
    // ⚠️ E vão com o prefixo `p.`: nesta consulta a coluna se chama `p.tr`, e sem qualificar
    // o Postgres escolhe sozinho ou recusa por ambiguidade — o primeiro caso é o que erra
    // calado.
    const cTr = condicaoTr(tr, params, params.length + 1, 'p.');
    if (cTr) w.push(cTr.condicao);
    const cProc = condicaoProcesso(processo, params, params.length + 1, 'p.');
    if (cProc) w.push(cProc.condicao);
...
/** As duas decisões, pelo rótulo da opção que as produz — o mesmo do rádio (lib/ci.js, OPCOES). */
const ROTULO_DECISAO = {
  de_acordo: require('./ci').OPCOES.de_acordo.rot,
  ressalva:  require('./ci').OPCOES.com_pendencia.rot,
};
