...
async function decidir(db, { codigos_pc, decisao, texto, autor }) {
  ...
  try {
    ...
    // (tr, parcial_num) e não tem coluna `codigo_pc`, e uma parcela pode ter 7 PCs decididas
    // em cliques diferentes.
    for (const r of alvo.rows) {
      await cli.query(
        `INSERT INTO parcela_historico (tr, parcial_num, setorial_id, evento, valor_anterior,
         ...
         SELECT p.tr, p.parcial_num, p.setorial_id, 'ci_decidiu', 'na_fila', $2::text,
                $3::int, NULL, $4::text, NOW()
           FROM prestacoes_contas p WHERE p.codigo_pc = $1`,
        [r.codigo_pc,
         decisao === 'de_acordo' ? 'encerrado' : 'com_analista',
         autor?.id ?? null,
         `${autor?.nome || 'C.I.'} decidiu no Controle Interno sobre a PC ${r.codigo_pc}: ` +
         (decisao === 'de_acordo'
...
function textoReabertura(autorNome, codigoPc, motivo) {
  return `${autorNome || 'C.I.'} reabriu no Controle Interno a PC ${codigoPc}: `
 ...
 */
async function reabrir(db, { codigos_pc, texto, autor }) {
  const cli = await db.connect();
  try {
    await cli.query('BEGIN');
    ...
    const alvo = await cli.query(SQL_REABRIR_ALVO, [codigos_pc]);
    if (!alvo.rows.length) {
      await cli.query('ROLLBACK');
...
module.exports = {
  ...
  SQL_REABRIR_ALVO, SQL_REABRIR, SQL_REABRIR_HISTORICO,
};
