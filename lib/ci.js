...
// ⚠️ AS DUAS OPÇÕES DO C.I. — as MESMAS na decisão e na reabertura (Richard, 14/09/2026). Eram
// dois rótulos longos na decisão e texto livre na reabertura, e o analista às vezes recebia só
// "com parecer". Agora o técnico escolhe uma das duas nos dois fluxos, e o apoio dela é a
// mensagem padrão: sem complemento, é ele que vai para a ci_mensagem.
// `decisao` é o que a opção faz com o estado na decisão: de acordo encerra, com pendência
// devolve (o ramo 'ressalva'). Na reabertura a parcial volta à analista com qualquer das duas.
const OPCOES = {
  de_acordo: { rot: 'De acordo', decisao: 'de_acordo',
    apoio: 'O C.I. concorda com o parecer. A analista confere o registro no SGPe e arquiva a parcial.' },
  com_pendencia: { rot: 'Com pendência', decisao: 'ressalva',
    apoio: 'Falta providência. A analista confere o processo no SGPe, resolve, e então arquiva ou responde ao C.I.' },
};
const MSG_SEM_OPCAO = 'Escolha uma das duas opções — De acordo ou Com pendência. O complemento sozinho não registra a decisão.';

const ehOpcao = (x) => typeof x === 'string' && Object.prototype.hasOwnProperty.call(OPCOES, x);

/** A opção pedida. Aceita o `decisao` antigo na decisão, para a tela que ainda não recarregou. */
function opcaoDe(b) {
  if (b && ehOpcao(b.opcao)) return b.opcao;
  if (b && b.decisao === 'de_acordo') return 'de_acordo';
  if (b && b.decisao === 'ressalva') return 'com_pendencia';
  return null;
}

/** O texto de uma mensagem do C.I. é complemento quando não é o próprio apoio da opção. */
function complementoDe(opcao, texto) {
  const t = String(texto || '').trim();
  if (!t) return null;
  return ehOpcao(opcao) && t === OPCOES[opcao].apoio ? null : t;
}
 ...
 * ─────────────────────────────────────────────────────────────────────────────
 */
async function gravarMensagem(db, codigosPc, { direcao, texto, autor, opcao }) {
  const op = ehOpcao(opcao) ? opcao : null;
  // Sem complemento, a mensagem do C.I. é o apoio da opção — a mensagem padrão.
  const t = (texto ?? '').toString().trim() || (op ? OPCOES[op].apoio : '');
  if (!t) return 0;
  const { rows } = await db.query(
    `INSERT INTO ci_mensagem (codigo_pc, rodada, direcao, texto, autor_id, autor_nome, autor_perfil, opcao)
     SELECT p.codigo_pc, GREATEST(p.ci_rodada, 1), $2::text, $3::text, $4::int, $5::text, $6::text, $7::text
       FROM prestacoes_contas p
      WHERE p.codigo_pc = ANY($1)
     RETURNING id`,
    [codigosPc, direcao, t, autor?.id ?? null, autor?.nome ?? null, autor?.perfil ?? null, op]);
  return rows.length;
}
 ...
 */
async function decidir(db, { setorial_id, tr, parcial_num, decisao: decisaoPedida, opcao, texto, autor }) {
  // A opção manda; o `decisao` antigo ainda é aceito e vira a opção correspondente.
  const op = ehOpcao(opcao) ? opcao : opcaoDe({ decisao: decisaoPedida });
  const decisao = op ? OPCOES[op].decisao : decisaoPedida;
  ...
  try {
    ...
    await gravarMensagem(cli, codigos, { direcao: 'ci_para_analista', texto, autor, opcao: op });
    ...
    await cli.query(
      ...
      [tr, String(parcial_num), setorial_id,
       ...
       autor?.id ?? null,
       `${autor?.nome || 'C.I.'} decidiu no Controle Interno sobre a parcial ${parcial_num} ` +
       `(${codigos.length} PC${codigos.length > 1 ? 's' : ''}: ${codigos.join(', ')}): ` +
       (op ? `${OPCOES[op].rot}. ${OPCOES[op].apoio}` : '') +
       (String(texto || '').trim() ? `\nComplemento: ${String(texto).trim()}` : '')]);
 ...
 */
function textoReabertura(autorNome, alvo, motivo, opcao) {
  // Pela rota vem a opção (14/09/2026) e o texto é complemento; pelo script de correção em lote
  // não há opção, o texto é o motivo e a origem é sempre o processo que voltou pelo SGPe.
  const op = ehOpcao(opcao) ? OPCOES[opcao] : null;
  return `${autorNome || 'C.I.'} reabriu no Controle Interno: ${alvo}. `
       + (op ? 'A parcial volta ao analista. ' : 'O processo voltou pelo SGPe depois do encerramento e volta ao analista. ')
       + 'A baixa e o encaminhamento ao C.I. seguem valendo.'
       + (op ? `\nOpção: ${op.rot}. ${op.apoio}` : '')
       + (String(motivo || '').trim() ? `\n${op ? 'Complemento' : 'Motivo'}: ${String(motivo).trim()}` : '');
}
 ...
 */
async function reabrir(db, { setorial_id, tr, parcial_num, codigos_pc, texto, autor, opcao }) {
  const cli = await db.connect();
  try {
    await cli.query('BEGIN');
    ...
    await gravarMensagem(cli, codigos, { direcao: 'ci_para_analista', texto, autor, opcao });
    ...
    if (porParcela) {
      await cli.query(SQL_REABRIR_HISTORICO_PARCELA,
        [tr, String(parcial_num), setorial_id, autor?.id ?? null,
         textoReabertura(autor?.nome, `parcial ${parcial_num} (${codigos.length} PC${codigos.length > 1 ? 's' : ''}: ${codigos.join(', ')})`, texto, opcao)]);
    } else {
...
module.exports = {
  ...
  OPCOES, MSG_SEM_OPCAO, ehOpcao, opcaoDe, complementoDe,
