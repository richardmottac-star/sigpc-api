...
// existiam TRÊS contagens de produtividade no sistema e elas não concordavam.
//
//   `GET /prestacoes_contas/produtividade` contava por `data_baixa` + `estornada`, e NÃO
//   olhava `baixada` nem `status`. A tela Produtividade (`prodCarregar`) contava
//   `status='baixada'`, e não olhava `estornada`.
//
...
//
// ⚠️ ATUALIZADO EM 27/08/2026 — A ROTA E A TELA VIRARAM UMA CONTA SÓ. As duas agora somam
// `sigef.SQL_CONTA_PRODUTIVIDADE` (a tela, pelo booleano `sigef_conta` que o servidor manda) e
// `sigef.sqlContaAte()` (a rota cumulativa). A base é `baixada = true OR enviado_ci = true`,
// menos as pendências de conferência com o SIGEF.
//
// ⚠️ MAS AS QUATRO COLUNAS CONTINUAM SENDO QUATRO. A conta unificada lê `baixada`,
// `enviado_ci`, `data_baixa` e `estornada` — quem quiser tirar uma PC da produtividade ainda
// precisa passar por aqui. Unificar a LEITURA não simplificou a ESCRITA.
//
  ...
  if (ciJaSeManifestou(pc))
    return {
      ...
      motivo: pc.ci_situacao === 'encerrado'
        ...
        + 'ela não pode ser puxada de volta. Responda ao C.I. pelo cartão da parcial.',
    };
