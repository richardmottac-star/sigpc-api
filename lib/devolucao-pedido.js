 ...
 *
 * ⚠️ O SOLICITANTE NÃO DECIDE O PRÓPRIO PEDIDO — decisão do Richard, 13/08/2026. Apareceu no
 * primeiro ciclo real: o pedido foi criado e aprovado pela mesma conta, e nada objetou.
 *
 * EXCEÇÃO: o superadmin decide o próprio, porque não há ninguém acima dele. Quando isso
 * acontecer, `autodecidido()` marca o registro — ver o histórico e o cartão da fila.
 *
 * @param quem   o usuário que está decidindo, lido do BANCO
 * @param pedido a linha de `solicitacao_devolucao` (precisa de `analista_id` e `analista_grupo`)
 */
function podeDecidir(quem, pedido) {
  if (!quem || !pedido) return false;
  const proprio = String(pedido.analista_id ?? '') === String(quem.id ?? '');
  if (quem.perfil === 'superadmin') return true;          // decide o próprio, e fica registrado
  if (proprio) return false;                              // coordenador não decide o dele
 ...
 *
 * Não é coluna nova: sai de `decidido_por = analista_id`, que o banco já guarda. Coluna
 * separada seria uma segunda fonte para a mesma resposta, e as duas divergiriam.
 */
...
// O texto que vai no histórico quando o superadmin decide o próprio pedido. Fica no registro,
// não só na tela: a tela alguém deixa de abrir.
...
function avisoPedido(pcs) {
  ...
  return {
    ...
    texto_baixadas: r.baixadas > 0
      ? `${r.baixadas} ${r.baixadas > 1 ? 'PCs baixadas permanecem' : 'PC baixada permanece'} `
        + 'no seu nome — a produtividade não é afetada.'
      : 'Nenhuma parcial baixada nesta TR.',
    texto_ci: r.no_ci > 0
      ? `${r.no_ci} ${r.no_ci > 1 ? 'PCs no Controle Interno BLOQUEIAM' : 'PC no Controle Interno BLOQUEIA'} `
...
const SQL_LISTAR = `
  SELECT s.*, u.nome AS analista_nome, u.grupo AS analista_grupo,
         ...
         (SELECT COUNT(DISTINCT p.tr) FROM prestacoes_contas p
           WHERE p.analista_id = s.indicado_id AND p.baixada = false
             AND ${inval.ativa('p')})::int AS indicado_ocupadas,
...
module.exports = {
  ...
  destinoAprovacao, impedimentoIndicado, autodecidido, MARCA_AUTODECIDIDO,
};
