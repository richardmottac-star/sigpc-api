...
async function contarOcupadas(db, analistaId, liberacao) {
  ...
  const { rows } = await db.query(
    `SELECT COUNT(DISTINCT tr)::int n FROM prestacoes_contas
      WHERE analista_id = $1 AND baixada = false`, [analistaId]);
  return rows[0].n;
}
...
async function situacao(db, usuario) {
  ...
  const limite = limiteBase + extras;
...
async function podeAssumirTr(db, usuario, tr) {
  ...
  const { rows } = await db.query(
    `SELECT 1 FROM prestacoes_contas WHERE tr = $1 AND analista_id = $2 LIMIT 1`,
    [tr, usuario.id]);
...
module.exports = {
  CONFIG_PADRAO, lerConfig, contarOcupadas, contarVagasExtras,
