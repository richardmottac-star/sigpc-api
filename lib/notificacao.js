...
// A regra dos dois papéis do superadmin. Sem ciclo: `papel.js` não requer ninguém.
const papel = require('./papel');

// ⚠️ 'repasse' ENTROU EM 01/09/2026, e a coluna `tipo` NÃO TEM CHECK no banco — conferido
// antes de escrever, como o `parcela_historico.evento` da transferência. Nenhum ALTER, nenhuma
// tabela nova: o repasse usa a mesma `notificacao` de sempre.
//
// ⚠️ E É TIPO PRÓPRIO, e não 'recado'. O tipo é o que a tela lê para decidir o ícone e os
// botões do aviso — enfiar o repasse em 'recado' obrigaria o front a adivinhar pelo texto do
// título, que é o tipo de dedução que este arquivo existe para não fazer.
const TIPOS = ['aprovacao', 'prazo', 'diligencia', 'recado', 'repasse'];

/**
 * QUEM PODE LER AS NOTIFICAÇÕES DE QUEM.  (24/08/2026)
 *
 * ⚠️ ATÉ HOJE NINGUÉM CONFERIA. `GET /notificacao?destinatario_id=X` devolvia a caixa de
 * qualquer pessoa a quem soubesse um id — sem senha, sem sessão, sem nada. Medido contra a
 * produção no mesmo dia: a chamada crua trouxe os avisos de diligência da Sandra (id 19),
 * com entidade, TR e prazo. Estava nas Pendências desde 13/08 e nada explorava, porque o
 * front sempre mandava o PRÓPRIO id.
 *
 * O que mudou o peso disso foi o sino passar a seguir o `alvo()`: a partir dali o cliente
 * oficial manda o id de OUTRA pessoa, e a rota passou a ser exercitada exatamente do jeito
 * que a deixava aberta. Fechar virou parte da mesma frente.
 *
 * A REGRA (decisão do Richard):
 *   · a pessoa lê as próprias, sempre;
 *   · o superadmin lê as de qualquer um — é ele quem age pela conta de outro;
 *   · coordenador e analista, só as próprias. ⚠️ O COORDENADOR **NÃO** LÊ AS DO GRUPO DELE.
 *     Ele decide sobre pedidos e vê a produtividade do grupo, mas a caixa de avisos é
 *     pessoal: tem recado do sininho e assunto que não é dele.
 *
 * ⚠️ `perfilEfetivo`, e não `perfil` cru — a regra única das rotas desde 14/08. No papel
 * analista o superadmin É analista, e aí lê só as próprias. Isso casa com o modo "agir pela
 * conta de", que já exige o papel técnico (`verComoPodeVer`): quem consegue entrar no modo
 * consegue ler, e quem não consegue, não.
 */
function podeLer(quem, destinatarioId) {
  if (!quem) return false;
  const id = parseInt(destinatarioId);
  if (!id) return false;
  if (Number(quem.id) === id) return true;
  return papel.perfilEfetivo(quem) === 'superadmin';
}
...
/**
 * TODOS os coordenadores EM EXERCÍCIO, dos três grupos.  (01/09/2026)
 *
 * ⚠️ NÃO É A `coordenadoresDoGrupo` COM OUTRO NOME, e a diferença é de regra, não de SQL:
 * aquela responde "quem decide sobre este grupo" e cai para o superadmin quando o grupo não
 * tem coordenador — porque um pedido sem destinatário fica sem resposta. Esta responde "quem
 * precisa saber", e a ordem do Richard é o GT inteiro: analista novo num grupo é assunto dos
 * três. Sem queda para o superadmin: aqui ninguém tem de decidir nada, e um aviso a menos não
 * trava operação nenhuma.
 *
 * ⚠️ "EM EXERCÍCIO" É `data_saida IS NULL`, NUNCA `ativo`. Os dispensados continuam com
 * `ativo = true` por decisão do Richard — quem saiu precisa terminar o que ficou em curso —,
 * e deduzir a dispensa do `ativo` mandaria aviso a quem já não está lá. É a mesma dupla de
 * colunas que a rota de transferência confere no destino.
 */
async function coordenadoresEmExercicio(db) {
  try {
    const { rows } = await db.query(
      `SELECT id FROM usuarios WHERE perfil = 'coordenador' AND data_saida IS NULL ORDER BY id`);
    return rows.map((r) => r.id);
  } catch (e) { return []; }
}

module.exports = {
  TIPOS, podeLer, criar, criarVarios, listar, contarNaoLidas, limparLidas,
  marcarLida, marcarTodas, coordenadoresDoGrupo, coordenadoresEmExercicio,
};
