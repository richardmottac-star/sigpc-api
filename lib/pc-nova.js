...
// ⚠️ O FILTRO AQUI É INVERTIDO, E A AUSÊNCIA É DELIBERADA — fase 2, 02/09/2026.
//
// As outras 26 consultas da fase 2 ganharam `inval.ativa()`. Esta NÃO pode ganhar: se a
// conferencia de duplicidade nao enxergar a PC invalidada, o cadastro RECRIA exatamente a PC
// que acabou de ser tirada de circulacao — e a nova nasce sem a marca, entao nada impediria
// de recria-la de novo. A duplicidade tem de ver o que a contagem nao ve.
//
// A constante existe para isso ser GREPAVEL: quem varrer o arquivo atras de consultas sem
// filtro acha esta e le por que ela fica como esta, em vez de "corrigi-la".
const SQL_JA_EXISTE = `SELECT codigo_pc, tr, tipo FROM prestacoes_contas
   WHERE codigo_pc = $1 AND ${inval.TODAS_INCLUSIVE_INVALIDADAS}`;
