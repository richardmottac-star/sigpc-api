...
const PADRAO = {
  ...
  // Manutenção mora na mesma linha de config_sistema e é lida na mesma ida ao banco: toda
  // tela já chama GET /config_sistema ao entrar, e um segundo SELECT para a mesma linha
  // seria só mais uma consulta com a mesma resposta. A REGRA dela vive em lib/manutencao.js.
  modo_manutencao: false,
  mensagem_manutencao: null,
  atualizado_em: null,
...
async function ler(db) {
  try {
    const { rows } = await db.query(
      `SELECT modo_preparacao, mensagem, modo_manutencao, mensagem_manutencao,
    ...
    return {
      ...
      modo_manutencao: !!rows[0].modo_manutencao,
      // Aqui NÃO se aplica texto padrão: quem decide a frase é lib/manutencao.recusa, que
      // é quem sabe qual é o padrão dela. Guardar o valor cru deixa a tela distinguir
      // "o Richard não escreveu nada" de "o Richard escreveu isto".
      mensagem_manutencao: rows[0].mensagem_manutencao || null,
