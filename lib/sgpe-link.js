...
(function (raiz) {
  ...
  // abaixo o rejeita.
  //
  // ⚠️ O bloco de dígitos é capturado CRU, com os zeros à esquerda. Antes ele vinha depois de
  // um `0*` que comia os zeros, e isso furava a trava: em "SDR05001028/2017" ela recebia
  // "5001028" e comparava com SDR50 (inexistente) em vez de SDR05 (que é órgão) — não disparava,
  // e o valor só não virava link errado porque SDR sozinho não está no mapa. Acidente, não
  // proteção. Os zeros agora saem DEPOIS da trava.
  ...
  function normalizarProcesso(bruto) {
    ...
    const crus = m[4];              // dígitos COMO ESCRITOS, zeros à esquerda inclusive
    ...
    //
    // Avalia `crus`, NUNCA o número já sem zeros: é o zero à esquerda que carrega a região.
    // A regra sai do mapa, não de sigla chumbada — se um dia entrar uma chave XXX05, todo
    // "XXX05.../ANO" colado passa a ser rejeitado sozinho, sem tocar neste código.
    if (regiao === undefined && separador === ''
        && !ORGAOS[letras] && ORGAOS[letras + crus.slice(0, 2)]) {
      return null;
    }

    // Só agora os zeros à esquerda saem. O limite de 8 dígitos significativos é o mesmo de
    // antes, quando o padrão trazia `0*(\d{1,8})`.
