...
// ⚠️ 'MANUAL' NÃO É SOBRESCRITO POR NENHUMA DAS TRÊS FUNÇÕES ABAIXO.
//
// É o link que um analista abriu no SGPe, copiou e colou — e ele só faz isso quando o
// automático NÃO resolveu. Deixar o job passar por cima apagaria o trabalho da pessoa em
// silêncio: o processo continuaria dando "não encontrado" e ninguém reclamaria, porque
// ninguém percebe um link que voltou a não existir.
//
// A trava existe em DOIS lugares, de propósito: `montarFila` (job_sgpe_links.js) não põe
// MANUAL na fila, e o `WHERE` daqui recusa a escrita mesmo se alguém chamar direto — por
// `--retentar-erros`, por script novo, ou por engano.
    ...
    `INSERT INTO sgpe_processo_ref
     ...
     ON CONFLICT (sigla, numero_oficial, ano) DO UPDATE
        SET nu_processo      = NULL,
            ...
            ultima_tentativa = NOW(),
            motivo           = $4
      WHERE sgpe_processo_ref.origem NOT IN ('CONFERIDO', 'SGPE', 'MANUAL')`,
    ...
    `INSERT INTO sgpe_processo_ref
     ...
     ON CONFLICT (sigla, numero_oficial, ano) DO UPDATE
        SET origem           = 'ERRO',
            ...
            ultima_tentativa = NOW(),
            motivo           = $4
      WHERE sgpe_processo_ref.origem NOT IN ('CONFERIDO', 'SGPE', 'NAO_ENCONTRADO', 'MANUAL')`,
    [p.sigla, p.numero, p.ano, String(motivo || '').slice(0, 300)]
  );
}
