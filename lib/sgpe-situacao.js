...
// ═══════════════════════════════════════════════════════════════════════════
//  O SETOR DO CONTROLE INTERNO NO SGPe  (23/09/2026)
//
// ⚠️ ISTO EXISTE PARA UMA PERGUNTA SÓ: "o processo está, agora, DENTRO do Controle Interno?".
// É ela que impede o sistema de mandar arquivar uma parcial cujo processo ainda está com o
// C.I. — medido em 23/09: 634 das 797 parciais que o sistema chamava de "prontas para
// arquivar" tinham o processo em FCEE/CONIN, alguma há 167 dias.
//
// ⚠️ A REGRA MORA AQUI, e não em quem pergunta. São dois chamadores (a leitura do estado e a
// rota que arquiva) mais o SQL — três cópias de um `LIKE '%CONIN%'` divergiriam no dia em que
// a FCEE criasse um segundo setor de controle, e a que ficasse para trás seria a que deixa
// arquivar.
//
// ⚠️ E A COMPARAÇÃO É PELA SIGLA DO SETOR, não pelo nome: o nome ("Controle Interno") é texto
// livre do portal e já vem com grafias diferentes entre processos; a sigla é estruturada.
const SETOR_CI = ['CONIN'];

/** A sigla do setor é de um setor do Controle Interno? Espelho em JS do `sqlSetorCI`. */
const ehSetorCI = (sigla) => {
  const s = String(sigla == null ? '' : sigla).toUpperCase();
  return SETOR_CI.some((c) => s.includes(c));
};

/** A mesma pergunta em SQL, sobre a expressão dada (ex.: `s.setor_sigla`). */
const sqlSetorCI = (expr) => '('
  + SETOR_CI.map((c) => `upper(coalesce(${expr},'')) LIKE '%${c}%'`).join(' OR ') + ')';
