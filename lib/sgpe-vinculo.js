...
function montar(linhas, chaveConsultada, papel) {
  ...
  for (const l of lista) {
    ...
    if (!mapa.has(bruto)) {
      mapa.set(bruto, {
        ...
        qtd: 0,
        parciais: new Set(),
        // ⚠️ VÊM DO `LEFT JOIN`, E `null` É RESPOSTA — quer dizer "ainda não sincronizado", e
        // não "sem assunto". A rota não inventa nem aproxima: processo que o job ainda não
        // visitou chega com os dois vazios, e é a tela que decide como dizer isso.
        // ⚠️ E são lidos da PRIMEIRA linha de cada processo. Todas as PCs de um mesmo
        // `processo_pc` casam a MESMA linha da `sgpe_situacao` — a chave é o processo, não a
        // PC —, então repetir a leitura em cada PC daria sempre o mesmo valor.
        assunto: l.assunto == null ? null : String(l.assunto),
        situacao_portal: l.situacao_portal == null ? null : String(l.situacao_portal),
  ...
  return {
    ...
    // ⚠️ A MÃE TAMBÉM LEVA OS DOIS, e vêm do SEU join (`mae_*`), não do da parcial. Ler o
    // `assunto` da primeira linha daria o assunto do processo_pc dela, que é outro processo.
    mae_assunto: l0.mae_assunto == null ? null : String(l0.mae_assunto),
    mae_situacao_portal: l0.mae_situacao_portal == null ? null : String(l0.mae_situacao_portal),
