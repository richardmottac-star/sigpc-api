...
const SQL_NL_PRIMEIRA_PARCELA = `(
  SELECT q.parcial_num FROM prestacoes_contas q
   WHERE q.tr = p.tr AND q.codigo_nl = p.codigo_nl AND ${inval.ativa('q')}
...
//
// ⚠️ NO VERMELHO E NO AZUL, QUALQUER DECLARAÇÃO VIRA CINZA. As três respostas dizem algo sobre
// o registro no SIGEF, e é exatamente o que essas duas perguntam.
//
// ⚠️ NO ÂMBAR, SÓ `nao_baixada` VIRA CINZA — e é a ÚLTIMA declaração que decide (11/09/2026).
// A âmbar afirma "o SIGEF baixou e aqui está aberta". `nao_baixada` é o analista dizendo que
// conferiu e o SIGEF NÃO baixou: a premissa da pílula é falsa, e ela sai. `ja_estava` e
// `registrei_agora` CONFIRMAM a premissa — e aí a pendência continua de pé, porque o que falta
// é o parecer AQUI. Apagar a pílula nesses dois casos esconderia uma PC aberta que o SIGEF já
// baixou. A última, e não qualquer uma, porque a declaração não se desmarca: quem errou
// declara de novo, e a mais recente é a que vale.
//
// ⚠️ Até esta data o SQL não apagava a âmbar nunca, e o JS apagava com qualquer resposta. As
// duas estavam erradas, cada uma para um lado; `classificar` repete esta regra.
const RESPOSTA_QUE_APAGA_AMBAR = 'nao_baixada';

// A resposta da declaração mais recente — `-> -1` é o último elemento do array jsonb.
const ULTIMA_RESPOSTA_SQL = `
  (CASE WHEN ${TEM_DECLARACAO} THEN p.sigef_declaracao -> -1 ->> 'resposta' END)`;
...
const SQL_TAG = `
  CASE
    ...
    WHEN ${PENDENCIA_SQL} = '${TAGS.ABERTA_COM_BAIXA_SIGEF}'
         AND ${ULTIMA_RESPOSTA_SQL} = '${RESPOSTA_QUE_APAGA_AMBAR}'
      THEN '${TAGS.REGISTRO_DECLARADO}'
    ELSE ${PENDENCIA_SQL}
  END`;
...
// ⚠️ PC NA ENGENHARIA NÃO CONTA (11/09/2026). Até aqui quem segurava isso era a PC não estar
// baixada — e a `2023PC001379` estava baixada E na engenharia, e contava. `IS DISTINCT FROM`
// porque `eng_situacao` é NULL em quase todas as linhas, e `NULL <> 'x'` é NULL: sem ele esta
// linha zeraria a produtividade inteira, pelo mesmo motivo do COALESCE acima.
const ENG_NA = 'na_engenharia';
const SQL_FORA_ENGENHARIA = `(p.eng_situacao IS DISTINCT FROM '${ENG_NA}')`;

const SQL_CONTA_PRODUTIVIDADE = `(
  ${SQL_BASE_PRODUTIVIDADE}
  AND COALESCE(${SQL_TAG}, '') NOT IN (${TAGS_QUE_DESCONTAM.map((t) => `'${t}'`).join(', ')})
  AND NOT ${SQL_PRE_GT}
  AND ${SQL_FORA_ENGENHARIA}
)`;
...
// ⚠️ A ENGENHARIA "ATÉ UMA DATA" — a MESMA forma do estorno (`estornada = false OR data_estorno
// > corte`) e da invalidação (`invalidada = false OR invalidada_em > corte`), copiada de
// propósito (11/09/2026). Sem ela o card descontava a PC na engenharia e o relatório do CGE
// não: a mesma PC contaria num e não no outro. Com o filtro simples, um relatório de agosto
// gerado hoje perderia a PC que só foi para a engenharia em setembro — o passado mudaria.
//
// ⚠️ O COALESCE cobre a PC na engenharia SEM `eng_enviada_em`: `false OR NULL` é NULL, e a
// resposta certa ali é "não conta", a mesma do card — que não olha data nenhuma.
const sqlForaEngenhariaAte = (p) =>
  `COALESCE(p.eng_situacao IS DISTINCT FROM '${ENG_NA}' OR p.eng_enviada_em > ${p}, false)`;

const sqlContaAte = (p) => `(
  ${sqlBaseAte(p)}
  AND COALESCE(${SQL_TAG}, '') NOT IN (${TAGS_QUE_DESCONTAM.map((t) => `'${t}'`).join(', ')})
  AND NOT ${SQL_PRE_GT}
  AND ${sqlForaEngenhariaAte(p)}
)`;
...
function classificar(pc) {
  ...
  if ((p === TAGS.SEM_REGISTRO_SIGEF || p === TAGS.VERIFICAR_FINAL) && temDeclaracao(pc))
    return TAGS.REGISTRO_DECLARADO;
  // ⚠️ Na âmbar, só a ÚLTIMA declaração, e só se ela for `nao_baixada` — ver o SQL_TAG.
  if (p === TAGS.ABERTA_COM_BAIXA_SIGEF && temDeclaracao(pc)
      && (pc.sigef_declaracao[pc.sigef_declaracao.length - 1] || {}).resposta === RESPOSTA_QUE_APAGA_AMBAR)
    return TAGS.REGISTRO_DECLARADO;
  return p;
}
...
function contaProdutividade(pc) {
  ...
  // ⚠️ A engenharia também — ver SQL_FORA_ENGENHARIA.
  if (pc && pc.eng_situacao === ENG_NA) return false;
...
module.exports = {
  ...
  SQL_BASE_PRODUTIVIDADE, SQL_CONTA_PRODUTIVIDADE, SQL_DESCONTADA, ENG_NA, SQL_FORA_ENGENHARIA,
  sqlBaseAte, sqlContaAte, sqlDescontadaAte, sqlForaEngenhariaAte, RESPOSTA_QUE_APAGA_AMBAR,
