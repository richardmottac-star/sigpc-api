...
// ⚠️ `PC_LIVRE_SQL` VEM DAQUI, e não é reescrito: é a definição ÚNICA de "livre" no sistema,
// a mesma do botão Assumir e da contagem `pcs_livres` da `resumo_tr`.
const { nomeCurto, PC_LIVRE_SQL } = require('./assumir');
...
// ══════════════════════════════════════════════════════════════════════════════
//  A ORIGEM "ESTOQUE" — encaminhar PC SEM DONO para um analista (22/09/2026)
// ══════════════════════════════════════════════════════════════════════════════
//
// Pedido do Richard: como superadmin, encaminhar do estoque direto para um analista quando a
// situação exigir. Antes disso o único caminho era ele mesmo assumir a TR e repassá-la — dois
// passos, e o primeiro deixava a marca de que ELE assumiu.
//
// ⚠️ O EVENTO CONTINUA SENDO `transferencia`, e o de/para é que muda: `valor_anterior` vira
// `— · estoque`. Um evento novo obrigaria a lista, o detalhe, o desfazer, a ciência e os
// avisos a aprender um segundo nome para a mesma coisa — e o DESFAZER já manda as PCs ao
// estoque, então ele é o inverso exato deste caminho, de graça.
//
// ⚠️ E "LIVRE" NÃO GANHA UMA SEGUNDA DEFINIÇÃO. Quem responde é `assumir.PC_LIVRE_SQL`, a
// mesma string do botão Assumir e da contagem `pcs_livres` da `resumo_tr`. Em 16/08 havia duas
// definições e 87 PCs caíam no vão entre elas (CLAUDE.md); escrever aqui um
// `analista_id IS NULL AND status = 'livre'` à mão seria reabrir o vão.
const ESTOQUE = 'estoque';
const ROTULO_ESTOQUE = '— · estoque';

/** O `de_id` pedido é o estoque? */
const ehEstoque = (v) => String(v == null ? '' : v).trim().toLowerCase() === ESTOQUE;

/**
 * O espelho em JS de `assumir.PC_LIVRE_SQL`, para ler a foto.
 *
 * ⚠️ A PARTE DA INVALIDADA NÃO SE REPETE AQUI porque a `SQL_FOTO` já filtrou: a foto só traz
 * PC ativa. Repetir seria afirmar duas vezes o que já está garantido — e a `ehPreGt` da
 * `lib/sigef.js` é o mesmo padrão: espelho do SQL, com o escopo do chamador respeitado.
 */
const ehLivre = (l) => !!l && l.analista_id === null && l.status === 'livre';

/** Valida o corpo. Devolve a mensagem de erro, ou null. */
function validar(b) {
  if (!b) return 'Nada informado.';
  const doEstoque = ehEstoque(b && b.de_id);
  const de = doEstoque ? 0 : parseInt(b.de_id) || 0;
  const para = parseInt(b.para_id) || 0;
  if (!doEstoque && !de) return 'Informe de quem sai o acervo.';
  ...
  //
  // ⚠️ E ELA NÃO SE APLICA AO ESTOQUE: ele não é pessoa, e `de` ali é 0 — sem esta ressalva,
  // um destino de id 0 (que não existe) seria a única forma de a conferência disparar.
  if (!doEstoque && de === para) return 'O analista de origem e o de destino são o mesmo.';
...
const SQL_FOTO = `
  SELECT codigo_pc, tr, parcial_num, setorial_id, analista_id, analista_nome, baixada,
   ...
   WHERE setorial_id = $1 AND tr = ANY($2::text[]) AND ${inval.ativa('')}
...
function trsAlheias(trsPedidas, linhasFoto, deId) {
  // ⚠️ `deId` FALSO É A ORIGEM ESTOQUE, e aí "ser da origem" é TER PC LIVRE. Uma TR só de PCs
  // baixadas, ou já com dono, é recusada pelo mesmo motivo da alheia: não há o que encaminhar.
  const daOrigem = new Set(
    (deId ? linhasFoto.filter((l) => l.analista_id === deId && !l.baixada)
          : linhasFoto.filter(ehLivre)).map((l) => l.tr)
  );
  return trsPedidas.filter((t) => !daOrigem.has(t));
}

/** As PCs que VÃO se mover: abertas, do `de_id`, nas TRs pedidas — ou as livres, do estoque. */
function pcsQueMovem(linhasFoto, deId) {
  if (!deId) return linhasFoto.filter(ehLivre);
 ...
 *
 * ⚠️ NO ESTOQUE ELA É VAZIA, e não é descuido: não há pessoa de origem cuja produtividade se
 * queira provar intacta. O que garante que nada mais se moveu é a outra metade da `conferir`,
 * que percorre a foto INTEIRA e cobra dono igual em tudo o que não estava na lista — inclusive
 * nas baixadas sem dono, que existem no acervo.
 */
...
// ── O UPDATE DO ESTOQUE ─────────────────────────────────────────────────────
//
// ⚠️ ELE É O `assumir.SQL_ASSUMIR` COM OUTRO FILTRO, e as três diferenças em relação ao
// `SQL_MOVER` acima são as que separam "trocar de dono" de "sair do estoque":
//   · `status = 'analise'` — a PC deixa de estar livre, senão ela ficaria com dono E livre ao
//     mesmo tempo, aparecendo no Estoque para qualquer um assumir por cima;
//   · `dt_inicio_analise = COALESCE(...)` — o relógio da análise começa agora, porque nunca
//     começou. No repasse entre analistas ele NÃO é tocado (a análise já corria);
//   · o filtro é `PC_LIVRE_SQL`, a definição única de livre, e não `analista_id = $3`.
//
// ⚠️ E O FILTRO REPETE A CONDIÇÃO DE LIVRE mesmo já tendo a foto, pela mesma razão do
// `SQL_MOVER`: entre a foto e o UPDATE cabe outra transação — um analista clicando "Assumir"
// na mesma TR. O `WHERE` é a garantia; a foto é a prova, e a conferência acusa a diferença.
const SQL_MOVER_ESTOQUE = `
  UPDATE prestacoes_contas
     SET analista_id = $3,
         analista_nome = $4,
         status = 'analise',
...
function paramsHistorico({ movidas, foto, deId, paraId, deNome, paraNome, usuarioId, motivo,
  ...
  return [
    ...
    rep(EVENTO),
    // ⚠️ A PONTA DE ORIGEM É `— · estoque` QUANDO NÃO HÁ PESSOA, e é o MESMO rótulo que o
    // desfazer já grava como destino. Os dois lendo a mesma string é o que faz o caminho de
    // ida e o de volta aparecerem como um par na trilha, em vez de dois fatos sem relação.
    rep(deId ? rotulo(deId, deNome) : ROTULO_ESTOQUE),
    rep(rotulo(paraId, paraNome)),
    rep(paraId),
    movidas.map((m) => (deId
      ? `${m.codigo_pc} — transferida de ${deNome} (id ${deId}) para ${paraNome} (id ${paraId}).`
      : `${m.codigo_pc} — encaminhada do estoque para ${paraNome} (id ${paraId}).`)
      + `${motivo ? ' ' + motivo : ''}`),
...
function conferir({ foto, depois, movidas, deId, paraId }) {
  ...
  for (const c of feitas) {
    ...
    // ⚠️ VINDA DO ESTOQUE, A PC TEM DE TER DEIXADO O `livre` — senão ela fica com dono E
    // livre, aparecendo no Estoque para outro analista assumir por cima do primeiro. É
    // exatamente o vão de 16/08, e a conferência é o que impede ele de voltar em silêncio.
    if (!deId && d.status === 'livre') p.push(`${c} continua livre depois do UPDATE`);
  }
...
function avisoDestino({ pcs, trs, deNome, quando, vencidas }) {
  return {
    ...
    // ⚠️ SEM `deNome` A ORIGEM É O ESTOQUE, e a frase muda de verbo: "repassadas de" pede uma
    // pessoa, e escrever "repassadas de estoque" faria o analista procurar um colega que não
    // existe. Quem vem do estoque foi ENCAMINHADA.
    mensagem: (deNome ? `Repassadas de ${deNome} em ${dataBr(quando)}. `
                      : `Encaminhadas do estoque em ${dataBr(quando)}. `)
...
function avisoCoord({ pcs, grupo, deNome, paraNome, quando }) {
  return {
    // ⚠️ O TÍTULO TAMBÉM MUDA, e não só o corpo: para a coordenação, "repasse" e
    // "encaminhamento do estoque" são fatos diferentes — um redistribui trabalho que já era de
    // alguém, o outro tira acervo parado da fila. Chamar os dois de repasse esconderia o
    // segundo dentro do primeiro na leitura rápida do sino.
    titulo: deNome ? `Repasse no Grupo ${grupo}` : `Encaminhamento do estoque no Grupo ${grupo}`,
    mensagem: deNome
      ? `${nPcs(pcs)} de ${deNome} passaram para ${paraNome} em ${dataBr(quando)}.`
      : `${nPcs(pcs)} do estoque passaram para ${paraNome} em ${dataBr(quando)}.`,
  };
}
...
module.exports = {
  ...
  ESTOQUE, ROTULO_ESTOQUE, ehEstoque, ehLivre, SQL_MOVER_ESTOQUE,
};
