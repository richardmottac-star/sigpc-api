...
setTimeout(async () => {
  try {
    ...
    console.log('\n═══ 8-B. AS ROTAS DA FILA DO C.I. EXISTEM E SAO GUARDADAS ═══');
    {
      ...
      //
      // ⚠️ AS ROTAS MUDARAM DE CAMINHO EM 25/08/2026, quando o C.I. voltou a ser POR PC:
      // `/ci/fila_trabalho` e as tres `/ci/tr/*` sairam, e no lugar ficaram `/ci/fila` e as
      // tres `/ci/pc/*`.
      const g = await pedir('/ci/fila?usuario_id=57');
      conf(g.status === 403, 'GET /ci/fila existe e recusa quem nao e do C.I.', `status ${g.status}`);
      ...
      const s = await pedir('/ci/fila?usuario_id=57&perfil=controle_interno');
      conf(s.status === 403, 'e mandar perfil na query nao ajuda ninguem a entrar', `status ${s.status}`);

      // Sem usuario nenhum: 401, e nao 500.
      const v = await pedir('/ci/fila');
      conf(v.status === 401, 'sem usuario_id devolve 401, e nao estoura', `status ${v.status}`);

      // ⚠️ A ROTA ANTIGA NAO PODE TER SOBRADO. Duas rotas com o mesmo caminho nao dao erro: o
      // Express casa na PRIMEIRA e a segunda vira codigo morto — e a antiga devolvia 200 sem
      // credencial nenhuma, para 1.737 PCs de uma vez.
      ...
      conf(!/app\.(get|post|patch)\('\/ci\/:/.test(src),
           'nao ha rota /ci/:param que possa engolir /ci/pc/...');
    }
